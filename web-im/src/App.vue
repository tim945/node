...
export default {
  ...
  components: {
    ...
    'c-dialog':() => import('@/components/dialog')
  },
  data(){
    return {
      title: '请选择群或者人员进行聊天',
  ...
  methods: {
    ...
    triggerGroup(item) {
      ...
      this.title = `你（${this.nickname}）正在和${item.name}群成员聊天`;
    },
    // 单聊
    triggerPersonal(item) {
      ...
      this.title = `你（${this.nickname}）正在和${item.nickname}聊天`;
    },
    send(){
    ...
    conWebSocket(){
      ...
      if(window.WebSocket){
        ...
        socket.onmessage = function(e){
          ...
          if(message.users) {
            vm.users = message.users; // 更新在线用户列表
          }
          if (message.groups){
            vm.groups = message.groups; // 更新群组列表
          }
